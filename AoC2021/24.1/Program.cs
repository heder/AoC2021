...
class Program
{
    static void Main()
    {
        ...
        sb.AppendLine("long w = 0;");
        sb.AppendLine("long x = 0;");
        sb.AppendLine("long y = 0;");
        sb.AppendLine("long z = 0;");
        ...
        foreach (var line in lines)
        {
            ...
            var param1 = parts[1]?.Trim();
            var param2 = (parts.Length == 3) ? parts[2]?.Trim() : "";

            switch(mnemonic)
            {
                case "inp": // Read an input value and write it to variable a.
                    sb.AppendLine($"{param1} = Convert.Toint32(input[{i}].ToString());");
                    i++;
                    break;

                case "add":
                    sb.AppendLine($"{param1} = {param1} + {param2};");
                    break;
                ...
                case "eql":
                    sb.AppendLine($"{param1} = ({param1} == {param2}) ? 1 : 0;");
                    break;
            }
        }

        var code = sb.ToString();
        System.IO.File.WriteAllText("code.txt", code);
